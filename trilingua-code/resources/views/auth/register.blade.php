...
@section('content')
    ...
    <div class="auth-logo">
        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M32 12 C 20 12, 12 20, 12 32 C 12 44, 20 52, 32 52" fill="#000"/>
        </svg>
    </div>
    ...
    <form method="POST" action="{{ route('register.store') }}" class="auth-form">
        ...
        <div class="form-field">
            <input id="name" name="name" value="{{ old('name') }}" type="text" placeholder="Full name" required autofocus />
            @error('name') <p class="error-message">{{ $message }}</p> @enderror
        </div>

        <div class="form-field">
            <input id="email" name="email" value="{{ old('email') }}" type="email" placeholder="Your email" required />
